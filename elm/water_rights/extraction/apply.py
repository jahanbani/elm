...
async def check_for_ordinance_info(doc, text_splitter, **kwargs):
    ...
    **kwargs
    ...
    if doc.metadata["contains_ord_info"]:
        ...
        doc.metadata["state"] = await StateExtractor(llm_caller).parse(doc)
...
async def extract_ordinance_text_with_llm(doc, text_splitter, extractor):
    ...
    doc.metadata["definition_ordinance_text"] = ordinance_text
...
async def extract_ordinance_values(doc, **kwargs):
    ...
    **kwargs
    ...
    if not doc.metadata.get("definition_ordinance_text"):
        msg = (
    ...
    text = doc.metadata["definition_ordinance_text"]
